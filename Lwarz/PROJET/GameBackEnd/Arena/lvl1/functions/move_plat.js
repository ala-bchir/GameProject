import{vpmove,cmpt, platforms} from '../../../global/lvl1/glb_var.js';
import {PlatformsMesure} from '../mesure.js';

function move_hb_plat(vali,i){ //vali = vleur de la plateforme i.
    if(PlatformsMesure[i].y >= vali - 100 && cmpt[0].platmove1 == 0){ // platforme monte ?
    ...
    if(PlatformsMesure[i].y <= vali + 100 && cmpt[0].platmove1 == 1){ // plateforme descent ?
        PlatformsMesure[i].y += vpmove;
        platforms[i].y+=vpmove;
        if(vali + 100 == PlatformsMesure[i].y){
            cmpt[0].platmove1 = 0;
